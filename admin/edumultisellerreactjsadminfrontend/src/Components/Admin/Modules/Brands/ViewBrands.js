...
const ViewBrands=()=>{
  ...
  const history=useHistory();
    ...
    useEffect(() => {
        axios.get(brandsroutes.viewbrands).then(resp=>{
    ...
    const deletebrand=(e,slug)=>{
      ...
      axios.delete(brandsroutes.deletebrand+slug).then(resp=>{
              switch (resp.data.status) {
                case 400:
    ...
    /**update brand status */
    const updatebrandstatus=(e,slug)=>{
      e.preventDefault();
      const thisclicked=e.currentTarget;
      
      axios.put(brandsroutes.updatebrandstatus+slug).then(resp=>{
        switch (resp.data.status) {
          case 400:
            swal("error",resp.data.msg,"error");
            break;
...
/**end of paging and sorting */
        ...
        brandslist=allbrands.map((brand)=>{
            return(
                <tr key={brand.id}>
                  ...
                  <td>{brand.shop.shop}</td>
                  <td> 
                       <Link style={{textDecoration:"none"}}  onClick={(e)=>updatebrandstatus(e,brand.slug)} to="javascript:void(0)" >{(brand.is_enabled==0 ? 'In Active':'Active')}</Link>
                   </td>
                   <td>
                   <Link to={`/admin/edit/edit-brand/${brand.slug}`} className="fas fa-edit" title="edit brand "></Link>&nbsp;&nbsp;&nbsp;
 
                   <Link to="javasscript:void(0)" onClick={(e)=>deletebrand(e,brand.slug)} className="fas fa-trash confirmdelete" title="trash brand"></Link>
                     </td>
  ...
  <section className="content">
    <div className="row">
     ...
     <Link to="/admin/view/trashed-brands" className="btn btn-success pull-right mt-1"><i className="fas fa-recycle"></i> Bin</Link>
