...
const EditProduct=(props)=>{
  ...
  const SaveProduct=(e)=>{
      ...
      axios.post(productsroutes.updateproduct+productslug,formData).then(resp=>{
  ...
  useEffect(() => {
    ...
    axios.get(productsroutes.editproduct+productlug).then(resp=>{
        switch (resp.data.status) {
            case 400:
                swal("warning",resp.data.msg,"warning")
                history.push("/admin/view/products");
      ...
      <div className="card card-default">
        ...
        <form  onSubmit={SaveProduct} role="form" encType="multipart/form-data">
          <div className="card-body">
            <div className="row">
              ...
              <div className={(error.productimage)? 'form-group text-danger':'form-group'}>
                  ...
                  <img src={`http://localhost:8000/Admin/Adminimages/ProductImages/small/${productInput.product_image}`} alt={productInput.category_name} width="50px" />
          ...
          <div className="card-footer">
            <button type="submit" className="btn btn-primary" onClick={(e)=>setbuttonText("Updating...")}>{buttonText}</button>
          </div>
        </form>
      </div>
    </div>
